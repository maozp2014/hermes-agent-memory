...
class MemoryStore:
    ...
    # ------------------------------------------------------------------
    # Phase 5 — Aggregated Stats (MEMORY.md + USER.md + holographic)
    # ------------------------------------------------------------------

    def stats(self, holographic_store=None) -> Dict[str, Any]:
        """Return a cross-store health report aggregating file + holographic stores.

        Reads MEMORY.md and USER.md from disk, then merges in holographic stats
        if a MemoryStore from holographic/store.py is provided.
        """
        import json as _json
        from datetime import datetime, timezone

        # ── MEMORY.md stats ──────────────────────────────────────────
        memory_entries = self._read_file(self._path_for("memory"))
        memory_chars = len(ENTRY_DELIMITER.join(memory_entries)) if memory_entries else 0
        memory_oldest = self._oldest_entry_age(memory_entries)

        # ── USER.md stats ────────────────────────────────────────────
        user_entries = self._read_file(self._path_for("user"))
        user_chars = len(ENTRY_DELIMITER.join(user_entries)) if user_entries else 0
        user_oldest = self._oldest_entry_age(user_entries)
        ...
        report: Dict[str, Any] = {
            ...
            "memory_md": {
                ...
                "char_count": memory_chars,
                "char_limit": self.memory_char_limit,
                "usage_pct": min(100, round(memory_chars / max(1, self.memory_char_limit) * 100, 1)),
                "oldest_entry_days": memory_oldest,
            },
            "user_md": {
                "entry_count": len(user_entries),
                "char_count": user_chars,
                "char_limit": self.user_char_limit,
                "usage_pct": min(100, round(user_chars / max(1, self.user_char_limit) * 100, 1)),
                "oldest_entry_days": user_oldest,
            },
        }

        # ── Holographic stats ─────────────────────────────────────────
        if holographic_store is not None:
            hstats = holographic_store.stats()
            report["holographic"] = hstats

        return report

    def _oldest_entry_age(self, entries: List[str]) -> Optional[float]:
        """Return age in days of the oldest entry by looking at file mtime.

        Returns None if no entries or file doesn't exist.
        """
        if not entries:
            return None
        mem_dir = get_memory_dir()
        paths = [mem_dir / "MEMORY.md", mem_dir / "USER.md"]
        for p in paths:
            if p.exists():
                import time
                mtime = p.stat().st_mtime
                now = time.time()
                return round((now - mtime) / 86400, 1)
        return None
...
def memory_tool(
    ...
    if action == "add":
        ...
        # Collect stats from both built-in store and external memory manager
        import json as _json
        report = store.stats()

        # Also pull external provider stats if a manager is wired in
        if memory_manager:
            try:
                ext_stats = memory_manager.get_stats()
                if ext_stats and ext_stats.get("providers"):
                    report["external_providers"] = ext_stats["providers"]
            except Exception:
                pass
        ...
        return tool_error(f"Unknown action '{action}'. Use: add, replace, remove, delete_pending, archive, list_backups, stats", success=False)
...
MEMORY_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["add", "replace", "remove", "delete_pending", "list_pending", "archive", "list_backups", "stats"],
